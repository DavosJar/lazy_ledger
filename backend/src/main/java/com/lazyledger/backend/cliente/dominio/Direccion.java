...
public record Direccion(String calle, String ciudad, String pais, String codigoPostal) {
    public Direccion {
        ...
        if (calle == null || calle.trim().isEmpty()) {
            throw new ValidationException("La calle no puede ser nula o vacía");
        }
        if (ciudad == null || ciudad.trim().isEmpty()) {
            throw new ValidationException("La ciudad no puede ser nula o vacía");
        }
        if (pais == null || pais.trim().isEmpty()) {
            throw new ValidationException("El país no puede ser nulo o vacío");
        }
        calle = calle.trim();
        ciudad = ciudad.trim();
        pais = pais.trim();
